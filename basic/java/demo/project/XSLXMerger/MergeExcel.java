...
public class MergeExcel {
    public static void main(String args[]) throws Exception {
        ...
        Workbook workBook1 = mergeExcel.checkFileFormat("D:\\VenkataMedapati\\BasicJava_Demos\\src\\basic\\java\\demo\\project\\XSLXMerger\\ABC.xls");
        Workbook workBook2 = mergeExcel.checkFileFormat("D:\\VenkataMedapati\\BasicJava_Demos\\src\\basic\\java\\demo\\project\\XSLXMerger\\ABCD.xls");
        String path = "D:\\VenkataMedapati\\BasicJava_Demos\\src\\basic\\java\\demo\\project\\XSLXMerger\\result.xls";
        ArrayList<ArrayListRows> rowsArrayList = mergeExcel
                .getObjectsFromWorkBook(workBook1);
        ArrayList<ArrayListRows> rowsArrayList1 = mergeExcel
                .getObjectsFromWorkBook(workBook2);
        ...
        ArrayList<Integer> tot = mergeExcel.compareHeadings(firstRowList1,
                firstRowList2);
        rowsArrayList1 = mergeExcel.sortColoumns(rowsArrayList1, tot);
        ArrayList<ArrayListRows> totalList = mergeExcel.addRows(rowsArrayList,
                rowsArrayList1);
    ...
    public void writeToFile(String path, ArrayList<ArrayListRows> totalList,
            ArrayListRows firstRowList1) throws Exception {
        ...
        for (int index = 0; index < firstRowList1.coloumns.size(); index++) {
            newTitleRow.createCell(index).setCellValue(
                    firstRowList1.coloumns.get(index));
        }
    ...
    public ArrayList<ArrayListRows> sortColoumns(
            ArrayList<ArrayListRows> rowsList, ArrayList<Integer> tot) {
    ...
    public ArrayList<Integer> compareHeadings(ArrayListRows firstRowList1,
            ArrayListRows firstRowList2) {
        ...
        for (int i = 0; i < firstRowList1.coloumns.size(); i++) {
            for (int j = 0; j < firstRowList2.coloumns.size(); j++) {
                if ((firstRowList1.coloumns.get(i).trim())
                        .equals((firstRowList2.coloumns.get(j).trim()))) {
                    tot.add(j);
                    break;
                }
            }
    ...
    public ArrayList<ArrayListRows> addRows(
            ArrayList<ArrayListRows> rowsArrayList,
            ArrayList<ArrayListRows> rowsArrayList1) {
                ...
                while (cells.hasNext()) {
                    cell = (Cell) cells.next();
                     if (cell.getCellType() == Cell.CELL_TYPE_BLANK) {
                        ...
                        allo.coloumns.add("" + cell.getBooleanCellValue());
                    }else if (cell.getCellType() == Cell.CELL_TYPE_FORMULA) {
                        allo.coloumns.add("" + cell.getCellFormula());
                    } else if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
                        allo.coloumns.add("" + cell.getNumericCellValue());
                    }else if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
                        allo.coloumns.add("" + cell.getStringCellValue());
                    }else
                        allo.coloumns.add("" + cell.getErrorCellValue());
                }
                arrayRows.add(allo);
            }
        }
    ...
    public Workbook checkFileFormat(String fileName) {
        ...
        try {
            ...
            return hwb;
                                        ...
                                        // XSSFWorkbook in case of class cast
                                        // exception
            Workbook xwb = null;
            // System.out.println("class cast");
            try {
                xwb = new HSSFWorkbook(checkFis);
                checkFis.close();
            }
            catch (IOException e) {
                e.printStackTrace();
        ...
        catch (Exception e) { // Instantiate the Workbook using XSSFWorkbook in
                              // case of Exception while reading file through
                              // HSSFWorkbook
            Workbook xwb = null;
            try {
                checkFis.close();
