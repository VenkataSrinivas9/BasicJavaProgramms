...
//        System.out.println("Time MilliSeconds: " + totalTime);
    ...
    public static int[] doInsertionSort(int[] input) {
        int temp;
        for (int i = 1; i < input.length; i++) { // Looping the array for sorting
            for (int j = i; j > 0; j--) { // Looping the array for Swaping
                if (input[j] < input[j - 1]) { // Finding the index of the smallest number
                    temp = input[j];
                    input[j] = input[j - 1];
                    input[j - 1] = temp;
                }
